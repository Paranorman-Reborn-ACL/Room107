//
//  TenantPendingChargesViewController.m
//  room107
//
...
@interface TenantPendingChargesViewController () <UITableViewDataSource, UITableViewDelegate>
...
@implementation TenantPendingChargesViewController
...
- (id)initWithRentedHouseItem:(RentedHouseItemModel *)rentedHouseItem {
    ...
    self = [super init];
    if (self != nil) {
        if (!rentedHouseItem || !rentedHouseItem.incomeOrders) {
        ...
        rentedHouseItem.incomeOrders = [rentedHouseItem.incomeOrders sortedArrayUsingDescriptors:[NSArray arrayWithObjects:descriptor, nil]];
        
        NSNumber *orderType = rentedHouseItem.incomeOrders[0][@"orderType"];
        NSMutableArray *incomeOrders = [[NSMutableArray alloc] init];
        for (NSDictionary *incomeOrder in rentedHouseItem.incomeOrders) {
...
- (UIView *)createFooterView {
    ...
    NSString *deadline = @"";
    double amount = 0;
    for (NSMutableDictionary *expenseOrder in _rentedHouseItem.incomeOrders) {
