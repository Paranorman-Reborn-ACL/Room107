//
//  TenantToPayBillsViewController.m
//  room107
//
...
@interface TenantToPayBillsViewController () <UITableViewDataSource, UITableViewDelegate>
...
@implementation TenantToPayBillsViewController
...
- (void)setRentedHouseItem:(RentedHouseItemModel *)rentedHouseItem {
    ...
    _sectionsTableView.hidden = YES;
    _payingTableView.hidden = YES;
    if ((!rentedHouseItem || !rentedHouseItem.expenseOrders) && !rentedHouseItem.paymentId) {
    ...
    if (rentedHouseItem.paymentId) {
        ...
        if (!rentedHouseItem.expenseOrders) {
            return;
        }
        ...
        rentedHouseItem.expenseOrders = [rentedHouseItem.expenseOrders sortedArrayUsingDescriptors:[NSArray arrayWithObjects:descriptor, nil]];
        
        _ordersString = @"";
        NSNumber *orderType = rentedHouseItem.expenseOrders[0][@"orderType"];
        NSMutableArray *expenseOrders = [[NSMutableArray alloc] init];
        for (NSDictionary *expenseOrder in rentedHouseItem.expenseOrders) {
...
- (void)changePaymentCost {
    ...
    if (_couponTextSwitch.isOn) {
        paymentCost -= [_rentedHouseItem.coupon intValue];
    }
    if (_balanceTextSwitch.isOn) {
        paymentCost -= [_rentedHouseItem.balance intValue];
    }
...
- (UIView *)createPayingTableHeaderView {
    ...
    [paymentCostView setPaymentInfo:[CommonFuncs moneyStrByDouble:(double)[_rentedHouseItem.paymentCost intValue] / 100] andDate:_rentedHouseItem.deadline];
...
- (UIView *)createFooterView {
    ...
    NSString *deadline = @"";
    _amount = 0;
    for (NSMutableDictionary *expenseOrder in _rentedHouseItem.expenseOrders) {
    ...
    [_couponTextSwitch setContent:[CommonFuncs moneyStrByDouble:[_rentedHouseItem.coupon doubleValue] / 100]];
    [footerView addSubview:_couponTextSwitch];
    if (0 == [_rentedHouseItem.coupon doubleValue]) {
        //红包为0 开关关闭
    ...
    [_balanceTextSwitch setContent:[CommonFuncs moneyStrByDouble:[_rentedHouseItem.balance  doubleValue] / 100]];
    
    if ([_rentedHouseItem.coupon intValue] >= _amount) {
        //红包够支付账单 并不需要余额 关闭开关 （优先使用红包）
        [_balanceTextSwitch setOn:NO];
    } else {
        //红包不够支付 采用余额
        if (0 == [_rentedHouseItem.balance  doubleValue]) {
            //余额为0 关闭按钮 不可打开
            [_balanceTextSwitch setOn:NO];
        } else {
            //有余额可用 打开按钮
...
- (UIView *)createPayingTableFooterView {
    ...
    if ([_rentedHouseItem.paymentCost intValue] >= maxPaymentCost) {
        CGFloat originX = 22;
        CGFloat originY = 11;
...
- (void)rightButtonDidClick {
    if (!_payingTableView || _payingTableView.hidden) {
        ...
        if (_couponTextSwitch.isOn) {
            _rentedHouseItem.couponCost = [_rentedHouseItem.coupon intValue] > _amount ? [NSNumber numberWithInt:_amount] : _rentedHouseItem.coupon;
        } else {
            _rentedHouseItem.couponCost = @0;
        }
        if (_balanceTextSwitch.isOn) {
            if (_couponTextSwitch.isOn) {
                if ([_rentedHouseItem.coupon intValue] >= _amount) {
                    _rentedHouseItem.balanceCost = @0;
                } else {
                    _rentedHouseItem.balanceCost = [NSNumber numberWithInt:_amount - _paymentCost - [_rentedHouseItem.coupon intValue]];
                }
            } else {
                _rentedHouseItem.balanceCost = [_rentedHouseItem.balance intValue] > _amount ? [NSNumber numberWithInt:_amount] : _rentedHouseItem.balance;
            }
        } else {
            _rentedHouseItem.balanceCost = @0;
        }
        _rentedHouseItem.paymentCost = [NSNumber numberWithInt:_paymentCost];
