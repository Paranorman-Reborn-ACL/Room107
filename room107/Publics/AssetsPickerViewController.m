...
- (void)viewDidAppear:(BOOL)animated {
    ...
    //解决iOS9后无法滚动至底部的问题
    if ([[UIDevice currentDevice].systemVersion floatValue] >= 9.0 && self.assets.count > 0) {
        [self.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:self.assets.count - 1 inSection:0] atScrollPosition:UICollectionViewScrollPositionTop animated:YES];
    }
}
...
- (void)setupAssets
{
    ...
    ALAssetsGroupEnumerationResultsBlock resultsBlock = ^(ALAsset *asset, NSUInteger index, BOOL *stop) {
        ...
        if (asset){
            ...
            if ([[UIDevice currentDevice].systemVersion floatValue] < 9.0 && self.assets.count > 0) {
                [self.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:self.assets.count - 1 inSection:0] atScrollPosition:UICollectionViewScrollPositionTop animated:NO];
            }
        }
    };
...
+ (void)initialize{
    ...
    checkedIcon     = [UIImage imageNamed:@"AssetsPickerChecked.png"];
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([self.type isEqual:ALAssetTypeVideo]){
        ...
        NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:self.title ? self.title : @""];
    ...
    if (self.selected){
        ...
        CGFloat iconWidth = checkedIcon.size.width / 2;
        CGFloat iconHeight = checkedIcon.size.height / 2;
        CGFloat offsetX = 3;
        CGFloat offsetY = 3;
        [checkedIcon drawInRect:(CGRect){CGPointMake(CGRectGetMaxX(rect) - iconWidth - offsetX, CGRectGetMaxY(rect) - iconHeight - offsetY), iconWidth, iconHeight}];
    }
}
