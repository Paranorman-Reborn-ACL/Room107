...
//    tagInputField_.placeholder = (_tagPlaceholder == nil) ? @"tag" : _tagPlaceholder;
    tagInputField_.attributedPlaceholder = [[NSAttributedString alloc] initWithString:_tagPlaceholder ? _tagPlaceholder : @"" attributes:@{NSForegroundColorAttributeName:[UIColor room107GrayColorB]}];
...
- (void)addSearchHandler:(void(^)())handler {
    ...
    tagInputField_.returnKeyType = UIReturnKeySearch; //将回车按钮定制为“搜索”
}
...
//            LogDebug(@"tagInputField_.x:%f, tagInputField_.width:%f, self.width:%f", tagInputField_.frame.origin.x, tagInputField_.frame.size.width, self.frame.size.width);
        } else {
            UIView *lastTag = tagSubviews_.lastObject;
    ...
    if (_mode == TLTagsControlModeEdit) {
        ...
        if (maxNum > 0) {
            if (_tags.count < maxNum) {
                ...
                [tagInputField_ setPlaceholder:_tags.count > 0 ? @"" : _tagPlaceholder];
            } else {
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if ([string isEqualToString:@" "]) {
        ...
        //回退键删除一个标签（确定当前没有正在输入的文本）
        if (_tags.count > 0 && [text isEqualToString:@""]) {
