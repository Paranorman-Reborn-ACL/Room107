...
//    LogDebug(@"index:%ld, -labelSize.width * 0.5:%f, inwardOffset.x:%f, -labelSize.height * 0.5:%f, inwardOffset.y:%f", index, -labelSize.width * 0.5, inwardOffset.x, -labelSize.height * 0.5, inwardOffset.y);
...
////        LogDebug(@"beginTracking bestGuessPoint.x:%f, bestGuessPoint.y:%f", bestGuessPoint.x, bestGuessPoint.y);
//        //防止闪动
...
//    LogDebug(@"continueTracking lastPoint:%f, lastPoint:%f", lastPoint.x, lastPoint.y);
...
-(void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    if(self.snapToLabels && self.innerMarkingLabels != nil)
    {
        ...
        self.angleFromNorth = floor([EFCircularTrig angleRelativeToNorthFromPoint:self.centerPoint
                                                                             toPoint:bestGuessPoint]);
//        LogDebug(@"endTracking bestGuessPoint.x:%f, bestGuessPoint.y:%f", bestGuessPoint.x, bestGuessPoint.y);
...
- (CGSize)sizeOfString:(NSString *)string withFont:(UIFont*)font
{
    ...
    return [[NSAttributedString alloc] initWithString:string ? string : @"" attributes:attributes].size;
}
...
//    LogDebug(@"angleFromNorth:%d, offset.x:%f, offset.y:%f", angleFromNorth, offset.x, offset.y);
