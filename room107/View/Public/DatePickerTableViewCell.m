...
- (void)setTips:(NSString *)tips {
    ...
    if (nil == tips || [tips isEqualToString:@""]) {
        [self resetNoTipsFrame];
    }
}
...
- (void)resetNoTipsFrame {
    [_tipsLabel removeFromSuperview];
    CGFloat originY = [self originTopY] + [self titleHeight];
    [_datePicker setFrame:CGRectMake([self originLeftX], originY, CGRectGetWidth([self cellFrame]) - 2 * [self originLeftX], 100)];
}
@end
