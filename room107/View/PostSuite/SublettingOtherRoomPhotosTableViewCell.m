//
//  SublettingOtherRoomPhotosTableViewCell.m
//  room107
//
...
@interface SublettingOtherRoomPhotosTableViewCell ()

@property (nonatomic, strong) UIView *containerView;

@end

@implementation SublettingOtherRoomPhotosTableViewCell
...
- (CGRect)cellFrame {
    return CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, sublettingOtherRoomPhotosTableViewCellHeight);
}
...
- (void)setOtherRoomPhotos:(NSMutableArray *)photos {
    ...
    for (NSMutableDictionary *photoDic in photos) {
        if (photoDic[@"value"] && ![photoDic[@"value"] isEqualToString:@""]) {
            ...
            SearchTipLabel *roomlabel = [[SearchTipLabel alloc] initWithFrame:(CGRect){imageViewX, labelY - 5, imageViewWidth, labelHeight}];
