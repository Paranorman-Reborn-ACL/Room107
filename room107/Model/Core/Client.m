...
- (AFHTTPRequestOperation *)authenticateWithRequest:(Request *)request requestPolicy:(ClientAuthRequestPolicy)requestPolicy completion:(RequestCompletionBlock)completion {
...
- (AFHTTPRequestOperation *)authenticateWithSavedRequest:(Request *)request {
...
- (AFHTTPRequestOperation *)performOperationkWithRequest:(Request *)request completion:(RequestCompletionBlock)completion {
...
- (AFHTTPRequestOperation *)enqueueOperationWithRequest:(Request *)request completion:(RequestCompletionBlock)completion {
    AFHTTPRequestOperation *operation = [self.HTTPClient operationWithRequest:request completion:completion];
    ...
    if (isAuthenticated) {
        ...
        for (AFHTTPRequestOperation *operation in self.pendingOperations) {
            if ([operation.request isKindOfClass:[NSMutableURLRequest class]]) {
                [(NSMutableURLRequest *)operation.request setAuthorizationHeaderWithOAuth2AccessToken:self.oauthToken.accessToken];
            }
        }
...
- (AFHTTPRequestOperation *)refreshTokenWithRefreshToken:(NSString *)refreshToken completion:(RequestCompletionBlock)completion {
...
- (AFHTTPRequestOperation *)refreshToken:(RequestCompletionBlock)completion {
