...
- (void)commonInit
{
    ...
    _font = [UIFont room107SystemFontThree];
    ...
    _hairline.backgroundColor = [UIColor room107ViewBackgroundColor];
...
- (void)setScrollOffset:(CGPoint)scrollOffset contentSize:(CGSize)contentSize
{
    ...
    if (offset == truncf(offset) && self.selectedSegmentIndex != index) {
        ...
        if ([self.delegate respondsToSelector:@selector(selectedSegmentIndexChanged:)]) {
            [self.delegate selectedSegmentIndexChanged:self];
        }
    }
...
{
    NSMutableString *mutableTitle = [NSMutableString stringWithString:title ? title : @""];
    ...
    if (self.showsCount) {
        ...
        NSString *resultString = self.inverseTitles ? [breakString stringByAppendingString:countString ? countString : @""] : [countString stringByAppendingString:breakString ? breakString : @""];
        
        [mutableTitle insertString:resultString atIndex:self.inverseTitles ? title.length : 0];
    }
    
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:mutableTitle ? mutableTitle : @""];
