...
- (void)record:(UInt32)offset {
	...
	if (error != nil) {
		LogDebug(@"up record json error %@ %@", key, error);
		return;
	}
	error = [_recorder set:key data:data];
	if (error != nil) {
		LogDebug(@"up record set error %@ %@", key, error);
	}
}
...
- (UInt32)recoveryFromRecord {
	...
	if (error != nil) {
		LogDebug(@"recovery error %@ %@", key, error);
	...
	if (t != _modifyTime) {
		LogDebug(@"modify time changed %llu, %llu", t, _modifyTime);
		return 0;
	}
