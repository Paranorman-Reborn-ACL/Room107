...
static const CGFloat kdisappearDuration = 0.5;//107动画消失时间
...
#pragma warning 改了barItem的frame（宽度和高度）
        BarItem *barItem = [[BarItem alloc] initWithFrame:CGRectMake(refreshControl.frame.origin.x, refreshControl.frame.origin.y, refreshControl.frame.size.width + 2, refreshControl.frame.size.height + 2) startPoint:startPoint endPoint:endPoint color:color lineWidth:lineWidth];
        barItem.tag = i;
...
{
    if (self.originalTopContentInset == 0) self.originalTopContentInset = self.scrollView.contentOffset.y;
...
- (void)finishingLoadingAndComplete:(void (^)(void))complete
{
